...
// we need the minimum of 3 * abs((a + b + c)/3 - median))
// let's simplify the equation 3 * abs((a + b + c) - median)
//                           = 3 * abs(((a + b + c) - 3 * median)/3)
//                           = 3 * abs(((a + b + c) - 3 * b)/3) [b = median, if a, b, c are order wised]
//                           = abs((a + b + c) - 3 * b)
// Now, use two pointers method and store the minimum difference to the answer everytime. For middle element, we use a loop.
