...
void check(vl& A, vl& ans, ll l, ll r, ll depth) {
    ll maxi = max_element(bg(A) + l, bg(A) + r) - bg(A);
    ans[maxi] = depth;
    if (l < maxi) check(A, ans, l, maxi, depth + 1);
    if (maxi < r - 1) check(A, ans, maxi + 1, r, depth + 1);
}
